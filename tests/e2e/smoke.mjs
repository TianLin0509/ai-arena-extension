...
try {
  ...
  check("manifest version_name = 5.2.25", manifest.version_name === "5.2.25", `actual: ${manifest.version_name}`);
  // v5.2.19+: 正式改名 AI圆桌派（v5.2.25 仍保持）
  ...
  check("sidepanel version badge", versionBadge === "v5.2.25", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v5.2.25"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  check("popup chat-version = v5.2.25", popupVersion === "v5.2.25", `actual: "${popupVersion}"`);
  ...
  const v52ApiRuntime = await popupPage.evaluate(() => ({
    ...
    hasNewerSelfTest: window.ChatUpdateCheck?._hasNewer?.("5.2.25", "v5.3.0-beta"),
    hasNewerSameTest: window.ChatUpdateCheck?._hasNewer?.("5.2.25", "v5.2.25"),
  }));
  check("v5.2.0 运行时: ChatUpdateCheck API 暴露 + currentVersion 返回 5.2.25 + hasNewer 比对逻辑正确",
    v52ApiRuntime.hasApi && v52ApiRuntime.hasCurrentVersion && v52ApiRuntime.hasNewerHelper &&
    v52ApiRuntime.curVer === "5.2.25" &&
