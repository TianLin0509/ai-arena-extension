...
try {
  ...
  check("manifest version_name = 4.3.3-beta", manifest.version_name === "4.3.3-beta", `actual: ${manifest.version_name}`);
  ...
  check("sidepanel version badge", versionBadge === "v4.3.3-beta", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v4.3.3-beta"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  check("popup chat-version = v4.3.3-beta", popupVersion === "v4.3.3-beta", `actual: "${popupVersion}"`);
