...
try {
  ...
  check("manifest version_name = 5.2.1-beta", manifest.version_name === "5.2.1-beta", `actual: ${manifest.version_name}`);
  ...
  check("sidepanel version badge", versionBadge === "v5.2.1-beta", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v5.2.1-beta"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  check("popup chat-version = v5.2.1-beta", popupVersion === "v5.2.1-beta", `actual: "${popupVersion}"`);
  ...
  // ── v5.2.1-beta polish: placeholder 动态化 + 重置弹窗美化 ──
  ...
  check("v5.2.1-beta polish ①: popup-task-menu 含 PLACEHOLDER_BY_TASK 4 模式 + updatePlaceholder",
  ...
  check("v5.2.1-beta polish ① 运行时: setTask('debate') 后 placeholder 含'辩论引导'+'留空'",
  ...
  check("v5.2.1-beta polish ① 运行时: 切回 ask 后 placeholder 恢复原文",
    placeholderRuntime.afterAsk.includes("Ctrl+Enter 发送") && placeholderRuntime.afterAsk.includes("@ 单发"),
    `actual: ${JSON.stringify(placeholderRuntime)}`);

  // v5.2.1-beta fix: ChatModal escListener 不再泄漏（点按钮关 modal 后 document 上无残留 keydown）
  ...
  check("v5.2.1-beta fix: 点按钮关 modal 后 Enter 不再泄漏触发 primary.onClick（修彻底重置 bug）",
  ...
  const v52ApiRuntime = await popupPage.evaluate(() => ({
    ...
    hasNewerSelfTest: window.ChatUpdateCheck?._hasNewer?.("5.2.1-beta", "v5.3.0-beta"),
    hasNewerSameTest: window.ChatUpdateCheck?._hasNewer?.("5.2.1-beta", "v5.2.1-beta"),
  }));
  check("v5.2.0 运行时: ChatUpdateCheck API 暴露 + currentVersion 返回 5.2.1-beta + hasNewer 比对逻辑正确",
    v52ApiRuntime.hasApi && v52ApiRuntime.hasCurrentVersion && v52ApiRuntime.hasNewerHelper &&
    v52ApiRuntime.curVer === "5.2.1-beta" &&
    v52ApiRuntime.hasNewerSelfTest === true &&
    v52ApiRuntime.hasNewerSameTest === false,
    JSON.stringify(v52ApiRuntime));

  // ── v5.2.1: 豆包 selector 修复（MCP 实测发现 Tailwind/Semi 命名） ──
  const selectorsV521 = fs.readFileSync(path.join(EXT_PATH, "selectors-config.js"), "utf8");
  check("v5.2.1: doubao response 含 bg-g-receive + receive-msg-bubble 等 Tailwind 风新 selector",
    /\[class\*="bg-g-receive"\]/.test(selectorsV521) &&
    /\[class\*="receive-msg-bubble"\]/.test(selectorsV521) &&
    /msg-bubble[^"]*"\]:not\(\[class\*="send"\]\)/.test(selectorsV521),
    "doubao selector 未加新命名兜底，会继续提取失败");
  check("v5.2.1: doubao input 含 textarea.semi-input-textarea（MCP 实测主输入框）",
    /textarea\.semi-input-textarea/.test(selectorsV521),
    "doubao input selector 缺 Semi Design 命名");
  check("v5.2.1: doubao userMessage 含 bg-g-send + send-msg-bubble-text",
