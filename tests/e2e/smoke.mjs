...
try {
  ...
  check("manifest version_name = 5.2.0a-beta", manifest.version_name === "5.2.0a-beta", `actual: ${manifest.version_name}`);
  ...
  check("sidepanel version badge", versionBadge === "v5.2.0a-beta", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v5.2.0a-beta"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  check("popup chat-version = v5.2.0a-beta", popupVersion === "v5.2.0a-beta", `actual: "${popupVersion}"`);
  ...
  // ── v5.2.0a-beta polish: placeholder 动态化 + 重置弹窗美化 ──
  ...
  check("v5.2.0a-beta polish ①: popup-task-menu 含 PLACEHOLDER_BY_TASK 4 模式 + updatePlaceholder",
  ...
  check("v5.2.0a-beta polish ① 运行时: setTask('debate') 后 placeholder 含'辩论引导'+'留空'",
  ...
  check("v5.2.0a-beta polish ① 运行时: 切回 ask 后 placeholder 恢复原文",
    placeholderRuntime.afterAsk.includes("Ctrl+Enter 发送") && placeholderRuntime.afterAsk.includes("@ 单发"),
    `actual: ${JSON.stringify(placeholderRuntime)}`);

  // v5.2.0a-beta fix: ChatModal escListener 不再泄漏（点按钮关 modal 后 document 上无残留 keydown）
  ...
  check("v5.2.0a-beta fix: 点按钮关 modal 后 Enter 不再泄漏触发 primary.onClick（修彻底重置 bug）",
  ...
  const v52ApiRuntime = await popupPage.evaluate(() => ({
    ...
    hasNewerSelfTest: window.ChatUpdateCheck?._hasNewer?.("5.2.0a-beta", "v5.3.0-beta"),
    hasNewerSameTest: window.ChatUpdateCheck?._hasNewer?.("5.2.0a-beta", "v5.2.0a-beta"),
  }));
  check("v5.2.0 运行时: ChatUpdateCheck API 暴露 + currentVersion 返回 5.2.0a-beta + hasNewer 比对逻辑正确",
    v52ApiRuntime.hasApi && v52ApiRuntime.hasCurrentVersion && v52ApiRuntime.hasNewerHelper &&
    v52ApiRuntime.curVer === "5.2.0a-beta" &&
