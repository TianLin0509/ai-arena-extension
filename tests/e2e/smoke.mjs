...
try {
  ...
  check("manifest version_name = 5.0.5", manifest.version_name === "5.0.5", `actual: ${manifest.version_name}`);
  // v5.2.19+: 正式改名 AI圆桌派（v5.0.5 特别Beta版 仍保持）
  ...
  check("sidepanel version badge", versionBadge === "v5.0.5", `actual: "${versionBadge}"`);

  const footerVersion = await sidepanelPage.locator(".footer").textContent();
  check("sidepanel footer version", footerVersion?.includes("v5.0.5"), `actual: "${footerVersion?.slice(0, 100)}"`);
  ...
  // v5.0.5: popup chat-version 改为对外文案"特别Beta版"，不再 mirror manifest 版本号
  ...
  const v52ApiRuntime = await popupPage.evaluate(() => ({
    ...
    hasNewerSelfTest: window.ChatUpdateCheck?._hasNewer?.("5.0.5", "v5.3.0-beta"),
    hasNewerSameTest: window.ChatUpdateCheck?._hasNewer?.("5.0.5", "v5.0.5"),
  }));
  check("v5.2.0 运行时: ChatUpdateCheck API 暴露 + currentVersion 返回 5.0.5 + hasNewer 比对逻辑正确",
    v52ApiRuntime.hasApi && v52ApiRuntime.hasCurrentVersion && v52ApiRuntime.hasNewerHelper &&
    v52ApiRuntime.curVer === "5.0.5" &&
