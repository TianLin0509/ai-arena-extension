...
(function (root) {
  const BUILTIN = {
    "debate.free": {
      ...
      name: "辩论 · 自由",
      category: "辩论",
      // v4.9.x: 增量契约（Claude 1 / Codex 2 / DeepSeek 3 三方迭代收敛）
      //   - 防稻草人：[明确反对] 必须先复述对方观点
      //   - 防重复：4 档增量契约（新增洞察/明确反对/修正判断/待验证）
      //   - 防空对空：每条 [明确反对] 必须含反例/数据/案例
      fields: [
        {
          key: "main",
          label: "主提示",
          value: `你正在参加一场多 AI 辩论。
下面是其他 AI 对同一问题的回答。

【发言前先做】
针对你打算反对的某个论点，先用一句话准确复述对方观点（让对方确认无误后再反驳）。
否则你是在打稻草人，整轮无效。

【你的输出必须是以下 4 类之一】（用前缀显式标注）

[新增洞察] 前轮无人提出的新视角、证据、反例
[明确反对] 针对某 AI 的具体论点 → 复述 + 给反例或更强证据
[修正判断] 我自己上轮的立场要改 → 改什么、为什么
[待验证]   提出一个能判定分歧的实验、数据、案例

【严禁】
- 复述自己上轮已说过的（增量原则）
- 礼貌性总结 / 泛泛表态 / "我赞同 XX 也想补充..."
- 纯抽象反对（每条 [明确反对] 必须有具体反例 / 数据 / 案例）`
        },
        {
          key: "r1",
          label: "第 1 轮引导",
...
请先表明你的核心立场，再给 2-3 条最强证据（数据/案例/原理 任一）。
不要试图驳倒别人——本轮目标是把"你站哪边、为什么"说清楚。`
        },
        {
          key: "r2",
          label: "第 2 轮引导",
          value: `第 2 轮（直接攻击对方最强论点的最弱证据）

不要再陈述自己立场（r1 已陈述过）。
本轮专攻：找出对手最强论点里最弱的那个证据，给反例或更强数据。
按 4 档增量契约输出（[明确反对] 类应该是本轮主导）。`
        },
        {
          key: "r3",
          label: "第 3 轮引导",
          value: `第 3 轮（明确让步 / 不让步 / 决策门槛）

本轮必须输出：
1. 我让步什么（被说服的部分，承认）
2. 我不让步什么（仍坚持的部分，理由）
3. 决策门槛（什么数据/实验/条件能让我改变看法）

不要试图"最后一击"——本轮目标是把分歧收敛到可判定状态。`
        }
      ]
    },
    "debate.collab": {
